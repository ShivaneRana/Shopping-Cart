...
function ColorSelector(){
    ...
    return(
        <div className={style.colorContainer}>
            ...
            className={style.colorHeadingHolder}>
                <p>Color</p>
                <img 
                    title={expanded ? "Shrink section" : "Expand section"}
                    src={expanded ? shrinkIcon : expandedIcon}></img>
            </div>
            <div className={style.colorItemHolder}>
                {
                    expanded ? colorList.map((item) => {
                        const tempUUID = uuidv7();
                        return <ColorItem key={tempUUID} color={item}></ColorItem>
                    }) : null
                }
            </div>
        </div>
...
function ColorItem({color}){
    ...
    return(
        <div className={style.colorItem}>
            <button onClick={toggleCheck} style={{backgroundColor:`${color}`}}>
                {
